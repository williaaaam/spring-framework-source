...
public interface AutowireCapableBeanFactory extends BeanFactory {
	...
	/**
	 * 目前通过包扫描@ComponentScan Spring默认的注入模型，也可以说默认情况下Spring是关闭自动注入，必须要我们通过setter方法或者构造函数完成依赖注入，并且Spring也不推荐修改默认配置。
	 * 自动注入模型
	 ...
	 * 或者修改bean#setAutowireMode(AutowireCapableBeanFactory.AUTOWIRE_BY_NAME)
	 * <p>
	...
	/**
	 * 指定的class创建一个Bean,这个Bean会经过属性注入，并且会执行相关的后置处理器，但是并不会放入Spring容器中
	 * <p>
