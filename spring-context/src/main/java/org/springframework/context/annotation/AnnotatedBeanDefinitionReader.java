...
public class AnnotatedBeanDefinitionReader {
	...
	//这个reader对象主要的工作就是注册BeanDefinition，那么将BeanDefinition注册到哪里去呢？所以它内部就保存了一个BeanDefinition的注册表。对应的就是我们代码中的AnnotationConfigApplicationContext
	...
	public AnnotatedBeanDefinitionReader(BeanDefinitionRegistry registry, Environment environment) {
		...
		// 从这个方法可以看出，Spring在创建reader对象的时候就开始注册bd了
	...
	private <T> void doRegisterBean(Class<T> beanClass, @Nullable String name,
		...
		// 同时将beanClass上注解提取到abd注解元数据属性metadata中
		...
		// 从注解元数据中提取 lazy-int,depends-on等信息到BeanDefinition
		AnnotationConfigUtils.processCommonDefinitionAnnotations(abd);
		if (qualifiers != null) {
			// 说我们手动直接注册了一个类，但是我们没有在类上添加@Lazy，@Primary注解，但是我们又希望能将其标记为Primary为true/LazyInit为true,这个时候就手动传入Primary.class跟Lazy.class即可
